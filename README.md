...
EZ2Play поддерживает базовую кастомизацию внешнего вида и звуков. Все файлы для кастомизации должны находиться в одной папке с `EZ2Play.exe`.

### Логотип

Чтобы изменить логотип заставки, подготовьте изображение `logo.png` и поместите его рядом с исполняемым файлом. Лаунчер автоматически использует его вместо встроенного логотипа.

### Звуковые пакеты

Вы можете заменить системные звуки, создав архив `sound.pack`. Для этого нужно создать обычный `.zip` архив и изменить его расширение на .pack.
...
> Файлы должны находиться в корне архива
